...
| Communication      | Expensive (IPC, pipes, sockets) | Cheap (shared variables)           |
| Creation Time      | Slow (due to separate memory)   | Fast                               |
| Crash Impact       | Crashes alone                   | Can crash entire process           |
| Resource Ownership | Has its own                     | Shares process’s resources         |
| Use Case           | Running different apps          | Doing tasks in parallel within app |


# Context Switching

- Context switching is the process where the CPU switches from one task (process or thread) to another, performed by Linux Scheduler.

- It’s what allows your computer to multitask — running a browser, media player, terminal, and background services all at once, even with a limited number of CPU cores.

- A context switch is when the CPU saves the state (context) of a running process/thread, and loads the state of the next one to be run.

**When does a context switch happen?**

- Context switching occurs when:
  - A higher-priority task becomes ready.
  - A task blocks (e.g., waiting for disk or network).
  - A time slice expires (preemptive multitasking).
  - An interrupt or system call requires the CPU to handle something else.

**Context**

- The context of a process/thread is everything the CPU needs to pause and resume it correctly later:

| Component             | What it includes                           |
| --------------------- | ------------------------------------------ |
| CPU Registers         | General-purpose registers (e.g., RAX, EAX) |
| Program Counter (PC)  | Tells where the program left off           |
| Stack Pointer         | Points to the top of the stack             |
| Memory Maps           | Layout of process memory                   |
| Open File Descriptors | Files opened by the process                |
| Scheduling Metadata   | Priority, time used, etc.                  |

**How it works – Simplified steps:**

- Pause current task
  - Save its CPU registers, PC, stack pointer into its process control block (PCB).
- Load next task
  - Load the CPU registers, PC, stack pointer from the next task’s PCB.
- Resume execution
  - CPU starts running the new task from where it left off.

🔁 This switch happens in microseconds, and can occur thousands of times per second.
